...
const quest_starting_pokemon = new (function () {
    ...
    const choice = async (loadString) => {
        ...
        newPokemon.addItemId('macho_brace')
        if (newPokemon.loadString == 'charmander')
            newPokemon.addItemId('fire_stone')
        if (newPokemon.loadString == 'bulbasaur')
            newPokemon.addItemId('leaf_stone')
        if (newPokemon.loadString == 'squirtle')
            newPokemon.addItemId('water_stone')
        if (newPokemon.loadString == 'pikachu')
            newPokemon.addItemId('thunder_stone')
...
const REGIONS = [
    ...
    new Region(1818, 1482, 32, 32, "viridian-forest", 20, ['normal']),
    new Region(1816, 1403, 32, 32, "pewter-city", 25, ['rock']),
    ...
    new Region(2104, 1340, 64, 64, "cerulean-city", 45, ['water']),
    new Region(2169, 1404, 32, 64, "kanto-route-5", 50, [], []),
    new Region(2168, 1533, 32, 64, "kanto-route-6", 55, [], []),
    new Region(2168, 1596, 32, 32, "vermilion-city", 60, ['electric']),
    new Region(2082, 1499, 84, 32, "kanto-route-7", 65, [], []),
    new Region(2016, 1499, 64, 32, "celadon-city", 70, ['grass']),
    new Region(2233, 1501, 32, 32, "kanto-route-8", 75, [], []),
    new Region(2168, 1470, 64, 64, "saffron-city", 100, ['psychic']),
    new Region(2264, 1500, 33, 32, "lavender-town", 100, [null]),
    new Region(2072, 1724, 32, 32, "fuchsia-city", 100, ['poison']),
    new Region(1816, 1788, 32, 32, "cinnabar-island", 100, ['fire']),
